/*
 * Protocol test vectors for the WireGuard core primitives.
 *
 * Provenance (AGENTS.md §5.6 — primary sources only):
 *  - X25519: RFC 7748 §5.2 "Test vector" block (fetched from
 *    rfc-editor.org 2026-08-24).
 *  - ChaCha20 block function: RFC 8439 §2.3.2.
 *  - AEAD_CHACHA20_POLY1305: RFC 8439 §2.8.2 (key/IV/AAD/plaintext/
 *    ciphertext/tag).
 */

#ifndef WG_VECTORS_H
#define WG_VECTORS_H

/* ---- RFC 7748 §5.2: X25519 Diffie-Hellman ---- */
#define VEC7748_ALICE_PRIV                                                     \
    { 0x77, 0x07, 0x6d, 0x0a, 0x73, 0x18, 0xa5, 0x7d, 0x3c, 0x16, 0xc1,       \
      0x72, 0x51, 0xb2, 0x66, 0x45, 0xdf, 0x4c, 0x2f, 0x87, 0xeb, 0xc0,       \
      0x99, 0x2a, 0xb1, 0x77, 0xfb, 0xa5, 0x1d, 0xb9, 0x2c, 0x2a }
#define VEC7748_ALICE_PUB                                                      \
    { 0x85, 0x20, 0xf0, 0x09, 0x89, 0x30, 0xa7, 0x54, 0x74, 0x8b, 0x7d,       \
      0xdc, 0xb4, 0x3e, 0xf7, 0x5a, 0x0d, 0xbf, 0x3a, 0x0d, 0x26, 0x38,       \
      0x1a, 0xf4, 0xeb, 0xa4, 0xa9, 0x8e, 0xaa, 0x9b, 0x4e, 0x6a }
#define VEC7748_BOB_PRIV                                                       \
    { 0x5d, 0xab, 0x08, 0x7e, 0x62, 0x4a, 0x8a, 0x4b, 0x79, 0xe1, 0x7f,       \
      0x8b, 0x83, 0x80, 0x0e, 0xe6, 0x6f, 0x3b, 0xb1, 0x29, 0x26, 0x18,       \
      0xb6, 0xfd, 0x1c, 0x2f, 0x8b, 0x27, 0xff, 0x88, 0xe0, 0xeb }
#define VEC7748_BOB_PUB                                                        \
    { 0xde, 0x9e, 0xdb, 0x7d, 0x7b, 0x7d, 0xc1, 0xb4, 0xd3, 0x5b, 0x61,       \
      0xc2, 0xec, 0xe4, 0x35, 0x37, 0x3f, 0x83, 0x43, 0xc8, 0x5b, 0x78,       \
      0x67, 0x4d, 0xad, 0xfc, 0x7e, 0x14, 0x6f, 0x88, 0x2b, 0x4f }
#define VEC7748_SHARED                                                         \
    { 0x4a, 0x5d, 0x9d, 0x5b, 0xa4, 0xce, 0x2d, 0xe1, 0x72, 0x8e, 0x3b,       \
      0xf4, 0x80, 0x35, 0x0f, 0x25, 0xe0, 0x7e, 0x21, 0xc9, 0x47, 0xd1,       \
      0x9e, 0x33, 0x76, 0xf0, 0x9b, 0x3c, 0x1e, 0x16, 0x17, 0x42 }

/* ---- RFC 8439 §2.3.2: ChaCha20 block function ---- */
#define VEC8439_BLOCK_KEY                                                      \
    { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,       \
      0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,       \
      0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f }
#define VEC8439_BLOCK_NONCE                                                    \
    { 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x00,       \
      0x00 }
#define VEC8439_BLOCK_COUNTER 1u
#define VEC8439_BLOCK_OUT                                                      \
    { 0x10, 0xf1, 0xe7, 0xe4, 0xd1, 0x3b, 0x59, 0x15, 0x50, 0x0f, 0xdd,       \
      0x1f, 0xa3, 0x20, 0x71, 0xc4, 0xc7, 0xd1, 0xf4, 0xc7, 0x33, 0xc0,       \
      0x68, 0x03, 0x04, 0x22, 0xaa, 0x9a, 0xc3, 0xd4, 0x6c, 0x4e, 0xd2,       \
      0x82, 0x64, 0x46, 0x07, 0x9f, 0xaa, 0x09, 0x14, 0xc2, 0xd7, 0x05,       \
      0xd9, 0x8b, 0x02, 0xa2, 0xb5, 0x12, 0x9c, 0xd1, 0xde, 0x16, 0x4e,       \
      0xb9, 0xcb, 0xd0, 0x83, 0xe8, 0xa2, 0x50, 0x3c, 0x4e }

/* ---- RFC 8439 §2.8.2: AEAD_CHACHA20_POLY1305 ---- */
#define VEC8439_AEAD_KEY                                                       \
    { 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a,       \
      0x8b, 0x8c, 0x8d, 0x8e, 0x8f, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95,       \
      0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d, 0x9e, 0x9f }
#define VEC8439_AEAD_NONCE                                                     \
    { 0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46,       \
      0x47 }
#define VEC8439_AEAD_AAD                                                       \
    { 0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6,       \
      0xc7 }
#define VEC8439_AEAD_PT                                                        \
    { 'L', 'a', 'd', 'i', 'e', 's', ' ', 'a', 'n', 'd', ' ', 'G', 'e',        \
      'n', 't', 'l', 'e', 'm', 'e', 'n', ' ', 'o', 'f', ' ', 't', 'h',        \
      'e', ' ', 'c', 'l', 'a', 's', 's', ' ', 'o', 'f', ' ', '\'', '9',       \
      '9', ':', ' ', 'I', 'f', ' ', 'I', ' ', 'c', 'o', 'u', 'l', 'd',        \
      ' ', 'o', 'f', 'f', 'e', 'r', ' ', 'y', 'o', 'u', ' ', 'o', 'n',        \
      'l', 'y', ' ', 'o', 'n', 'e', ' ', 't', 'i', 'p', ' ', 'f', 'o',        \
      'r', ' ', 't', 'h', 'e', ' ', 'f', 'u', 't', 'u', 'r', 'e', ',',        \
      ' ', 's', 'u', 'n', 's', 'c', 'r', 'e', 'e', 'n', ' ', 'w', 'o',        \
      'u', 'l', 'd', ' ', 'b', 'e', ' ', 'i', 't', '.' }
#define VEC8439_AEAD_CT                                                        \
    { 0xd3, 0x1a, 0x8d, 0x34, 0x64, 0x8e, 0x60, 0xdb, 0x7b, 0x86, 0xaf,       \
      0xbc, 0x53, 0xef, 0x7e, 0xc2, 0xa4, 0xad, 0xed, 0x51, 0x29, 0x6e,       \
      0x08, 0xfe, 0xa9, 0xe2, 0xb5, 0xa7, 0x36, 0xee, 0x62, 0xd6, 0x3d,       \
      0xbe, 0xa4, 0x5e, 0x8c, 0xa9, 0x67, 0x12, 0x82, 0xfa, 0xfb, 0x69,       \
      0xda, 0x92, 0x72, 0x8b, 0x1a, 0x71, 0xde, 0x0a, 0x9e, 0x06, 0x0b,       \
      0x29, 0x05, 0xd6, 0xa5, 0xb6, 0x7e, 0xcd, 0x3b, 0x36, 0x92, 0xdd,       \
      0xbd, 0x7f, 0x2d, 0x77, 0x8b, 0x8c, 0x98, 0x03, 0xae, 0xe3, 0x28,       \
      0x09, 0x1b, 0x58, 0xfa, 0xb3, 0x24, 0xe4, 0xfa, 0xd6, 0x75, 0x94,       \
      0x55, 0x85, 0x80, 0x8b, 0x48, 0x31, 0xd7, 0xbc, 0x3f, 0xf4, 0xde,       \
      0xf0, 0x8e, 0x4b, 0x7a, 0x9d, 0xe5, 0x76, 0xd2, 0x65, 0x86, 0xce,       \
      0xc6, 0x4b, 0x61, 0x16 }
#define VEC8439_AEAD_TAG                                                       \
    { 0x1a, 0xe1, 0x0b, 0x59, 0x4f, 0x09, 0xe2, 0x6a, 0x7e, 0x90, 0x2e,       \
      0xcb, 0xd0, 0x60, 0x06, 0x91 }

#endif /* WG_VECTORS_H */
